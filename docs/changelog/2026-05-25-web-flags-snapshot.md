...
- **Живой Brave-fetch не прогнан** — нет `BRAVE_API_KEY` локально. LLM-классификатор
  ПОДТВЕРЖДЁН реальным прогоном (`pytest -m network`, kimi-k2p6: верный закрытый enum +
  event_date + distractor→none). Не подтверждено только качество/свежесть реальной
  Brave-выдачи — перед demo задать ключ и прогнать CLI `detect`. Не блокер мёрджа кода.
...
- **Живая Brave-выдача не прогнана** (нет ключа). Классификатор подтверждён на
  реальном LLM (network-тест); единственный незакрытый риск — качество/свежесть самой
  Brave-выдачи на проде (классификатор хорош ровно настолько, насколько релевантны hits).
