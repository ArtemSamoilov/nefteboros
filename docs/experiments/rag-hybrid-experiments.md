...
Прогон 2026-05-25 на CUDA (RTX 2070), commit `015e1bb`. Коллекция `nefteboros_corpus_v2_heading` пересобрана с нуля (802 чанка, 7.5 мин embedding, 3 сек upsert). Конфиги: `bi` / `bi+hybrid` (RRF, k=60) / `bi+hybrid-weighted` (alpha=0.5). k_dense=30, k_sparse=30, k_final=10.

JSON-метрики: `metrics/runs/2026-05-25_rag_{dense_v2,hybrid_rrf,hybrid_weighted}_015e1bb.json`.
...
Sanity-check ✓: dense@v2 `chunk_hit@5 = 0.779`, `source_hit@5 = 0.989` — точно baseline из `rag-full-eval-report.md` §3.1.
...
### По блоку (chunk_hit@5)

| Блок | dense | +RRF | Δ RRF | +weighted | Δ wtd |
|---|---:|---:|---:|---:|---:|
...
1. **Цель `hybrid@v2 > 0.779` достигнута:** RRF даёт `0.832` (+5.3 п.п.) на overall `chunk_hit@5`, weighted — `0.863` (+8.4).
2. **Гипотеза «прирост на v2 < чем на v1» НЕ подтвердилась.** На corporate hybrid RRF на v2 даёт +28.6 п.п. (на v1 было +21.4) — БОЛЬШЕ, чем на raw-dense. На RU тоже больше: +22.5 на v2 vs +17.5 на v1. Heading-prefix и BM25 целятся в один кейс (SAME_DOC_MISS, корпоративные таблицы/тикеры), но их сигналы **дополняющие**, а не дублирующие: prefix даёт документный контекст, BM25 — точный лексический матч внутри документа.
3. **Появилась направленная регрессия там, где dense уже близок к потолку** — EN `−7.3` п.п., strategy `−5.1`, operational `−11.8`. На v1 такого паттерна не было (там BM25 не трогал сильные слайсы). На v2 prefix выжал EN/strategy (0.855/0.897), и теперь BM25 в этих случаях иногда вытесняет правильный чанк лексически похожим. Подтверждается и на overall `source_hit@5`: 0.989 → 0.968 (−2.1 п.п.). Эффект мал, но воспроизводимый. **На текущем 95-Q сабсете overall выигрыш перекрывает регрессию**; при росте EN/strategy доли в реальном трафике этот баланс может сместиться — нужен manual eval-сет.
4. **weighted > RRF (0.863 vs 0.832)** — повторяется паттерн с v1 (`§2 Ловушка`). На synthetic-сете weighted эксплуатирует пиковость BM25-скоров; на реальных запросах преимущество схлопнется. Production-default остаётся RRF.
5. **chunk_hit@1 выросла резко** — с 0.347 до 0.421 (RRF) / 0.516 (weighted). Это тоже отчасти symptom synthetic-overlap'а (BM25 ставит «лексический клон» вопроса на ранг 1), но абсолютный сдвиг полезен и для реального трафика, где первый чанк часто всё, что увидит LLM.
