...
- **Cost / tokens / model name в Langfuse generations — не обогащаются.** `log_llm_usage` вызывается в коде после каждого LLM-вызова (synthesize, llm_disambiguate), но в финальные generation-spans поля `provided_model_name`, `usage_details`, `total_cost` уходят как `null` (проверено дампом 10 generations за последние ~15 мин). Латентность и иерархия вызовов работают полноценно — рецензент видит дерево вызовов и время каждого узла. Полная реализация enrichment'а cost / tokens / model name — в плане v2.4.
