...
- [~] **Живой Brave-fetch НЕ прогнан** — нет `BRAVE_API_KEY` локально. LLM-классификатор
      подтверждён реальным прогоном (выше); НЕ подтверждено только качество/свежесть
      реальной Brave-выдачи. Перед demo — задать `BRAVE_API_KEY` и прогнать CLI `detect`.
      Не блокер мёрджа кода, блокер demo.
